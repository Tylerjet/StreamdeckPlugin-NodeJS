  ...
  _socket.on('message', (msg) => {
    ...
    switch (evt.event) {
      ...
      case 'setTitle':
        console.log(
          Chalk.green(
            `********TITLE SET: title: ${evt.payload.title} - Sending titleParametersDidChange event to plugin\n`,
          ),
        );
        json = {
          action: manifest.Actions[0].UUID.toLowerCase(),
      ...
      case 'setSettings':
        console.log(
          Chalk.green(`********SETTINGS STORED: ${JSON.stringify(evt.payload)} *******\n`),
        );
      ...
      case 'setGlobalSettings':
        console.log(
          Chalk.green(`********GLOBAL SETTINGS STORED: ${JSON.stringify(evt.payload)} *******\n`),
        );
      ...
      case 'setState':
        console.log(
          Chalk.green(`********SET STATE RECEIVED: ${JSON.stringify(evt.payload)} *******\n`),
        );
        break;
      case 'switchToProfile':
        console.log(
          Chalk.green(
            `********SWITCH TO PROFILE RECEIVED: ${JSON.stringify(evt.payload)} *******\n`,
          ),
        );
        break;
      case 'logMessage':
        console.log(
          Chalk.green(`********LOG MESSAGE RECEIVED: ${JSON.stringify(evt.payload)} *******\n`),
        );
        break;
      default:
...
process.on('message', (msg) => {
  const json = {
    action: manifest.Actions[actionIndex].UUID.toLowerCase(),
    event: '',
    context: '',
    device: config.server.deviceId,
    payload: {
