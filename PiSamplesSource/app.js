...
function connectElgatoStreamDeckSocket(
  ...
  var getApplicationName = function (jsn) {
    if (jsn && jsn['payload'] && jsn.payload['application']) {
      return isMac
        ? jsn.payload.application.split('.').pop()
        : jsn.payload.application.split('.')[0];
    }
    return '';
  };
  ...
  websocket.onmessage = function (evt) {
    try {
      ...
      if (~['applicationDidLaunch', 'applicationDidTerminate'].indexOf(event)) {
        ...
        const arrImages =
          event === 'applicationDidTerminate' ? [img, 'images/terminated.png'] : img;
        ...
        if (piContext && piContext !== 0) {
          // there's a property inspector
        ...
        if (jsonObj['event'] && jsonObj['event'] === 'willAppear') {
          ...
          bEvt = !jsonObj.hasOwnProperty('action')
            ? jsonObj.event
            : jsonObj.event + jsonObj['context'];
        }
    ...
    action['sendToPlugin' + jsn.context] = function (jsn) {
      ...
      if (jsn.hasOwnProperty('payload')) {
        ...
        if (pl.hasOwnProperty('property_inspector')) {
          ...
          console.log(
            '%c%s',
            'font-style: bold; color: white; background: blue; font-size: 15px;',
            `PI-event for ${jsn.context}:${pi}`,
          );
          ...
          if (pl.hasOwnProperty('sdpi_collection')) {
            console.log(
              '%c%s',
              'color: white; background: blue; font-size: 12px;',
              `PI SENDTOPLUGIN for ${jsn.context}`,
            );
...
function loadImage(inUrl, callback, inCanvas, inFillcolor) {
  ...
  const canvas =
    inCanvas && inCanvas instanceof HTMLCanvasElement ? inCanvas : document.createElement('canvas');
